...
class MainViewModel(application: Application) : AndroidViewModel(application) {
    ...
    // 后台 Ping 支持（切换页面不中断）
    private val _backgroundPingOutput = MutableStateFlow(listOf<String>())
    val backgroundPingOutput: StateFlow<List<String>> = _backgroundPingOutput
    
    private val _backgroundPingTimes = MutableStateFlow(listOf<Double>())
    val backgroundPingTimes: StateFlow<List<Double>> = _backgroundPingTimes
    
    private val _isBackgroundPingRunning = MutableStateFlow(false)
    val isBackgroundPingRunning: StateFlow<Boolean> = _isBackgroundPingRunning
    
    private var backgroundPingJob: Job? = null
    
    fun startBackgroundPing(
        address: String,
        useICMP: Boolean,
        pingCount: Int,
        pingSize: Int,
        ...
        backgroundPingJob?.cancel()
        _backgroundPingOutput.value = emptyList()
        _backgroundPingTimes.value = emptyList()
        _isBackgroundPingRunning.value = true
        
        backgroundPingJob = viewModelScope.launch {
            ...
            fun generateSummary(): String {
                ...
                return buildString {
                    ...
                    appendLine("${transmitted.value} packets transmitted, $received received, ${kotlin.math.round(loss).toInt()}% packet loss")
                    if (received > 0) {
                        appendLine("rtt min/avg/max = ${kotlin.math.round(min).toInt()}/${kotlin.math.round(avg).toInt()}/${kotlin.math.round(max).toInt()} ms")
                    }
                }
            }
            ...
            try {
                if (useICMP) {
                    val flow = IcmpPing.ping(address, pingCount, pingSize)
                    flow.collect { line ->
                        _backgroundPingOutput.value = _backgroundPingOutput.value + line
                        val timeRegex = Regex("time[=<]\\s*([0-9.]+)\\s*ms")
                        timeRegex.find(line)?.let { match ->
                            match.groupValues[1].toDoubleOrNull()?.let { time ->
                                times.add(time)
                                _backgroundPingTimes.value = (_backgroundPingTimes.value + time).takeLast(50)
                            }
                        }
                    ...
                    var seq = 0
                    while (pingCount == 0 || seq < pingCount) {
                        seq++
                        transmitted.value++
                        val start = System.nanoTime()
                        val success = TcpPing.tcpConnect(address, pingPort, 2000)
                        val timeMs = (System.nanoTime() - start) / 1_000_000.0
                        val line = if (success) {
                            times.add(timeMs)
                            _backgroundPingTimes.value = (_backgroundPingTimes.value + timeMs).takeLast(50)
                            "Connected to $address:$pingPort seq=$seq time=${kotlin.math.round(timeMs).toInt()} ms"
                        } else {
                            "Connection timeout for seq=$seq"
                        }
                        _backgroundPingOutput.value = _backgroundPingOutput.value + line
                        if (pingCount == 0 || seq < pingCount) delay(1000)
                    }
                }
                _backgroundPingOutput.value = _backgroundPingOutput.value + generateSummary()
            } catch (e: CancellationException) {
                _backgroundPingOutput.value = _backgroundPingOutput.value + generateSummary()
                _backgroundPingOutput.value = _backgroundPingOutput.value + "\n--- Ping 已手动取消 ---"
            } catch (e: Exception) {
                _backgroundPingOutput.value = _backgroundPingOutput.value + "\n发生错误: ${e.message}"
            } finally {
                _isBackgroundPingRunning.value = false
                backgroundPingJob = null
            }
        }
    }
    
    fun stopBackgroundPing() {
        backgroundPingJob?.cancel()
        _isBackgroundPingRunning.value = false
    }
    
    fun clearBackgroundPingOutput() {
        _backgroundPingOutput.value = emptyList()
        _backgroundPingTimes.value = emptyList()
    }
}
